...
MyGuiMainFrame::MyGuiMainFrame(const TGWindow *p, int w, int h) : TGMainFrame(p, w, h)
{
    ...
    //1. Setting Text Tab
    {
...

        {
            ...
            fSettingText1->LoadBuffer(gMyGuiMainAction->GenerateBEAMSettingsText());
    ...
    //2. Buttons Tab
    {
        ...
        // 2.1 page0
        for (int i = 0; i < gMyGuiMainAction->GetNTabPage(); i++)
        {
            TGCompositeFrame *fTabPage = fTab0->AddTab(gMyGuiMainAction->GetTabPageName(i));
            fTabPage->SetLayoutManager(new TGVerticalLayout(fTabPage));

            for (int j = 0; j < gMyGuiMainAction->GetNPageButton(i); j++)
            {
                if (gMyGuiMainAction->GetPageButtonName(i, j) == "SEP")
                {
                    TGHorizontal3DLine *fHorizontal3DLine = new TGHorizontal3DLine(fTabPage, 102, 8);
                    fTabPage->AddFrame(fHorizontal3DLine, LayoutX);
                }
                else if (gMyGuiMainAction->GetPageButtonName(i, j) == "Slider")
                {
                    TGHSlider *fHSlider = new TGHSlider(fTabPage, 200, kSlider1 | kScaleBoth, gMyGuiMainAction->GetIndexButton(i) + j, kHorizontalFrame);
                    ...
                    sldList.insert(pair<int, TGSlider *>(gMyGuiMainAction->GetIndexButton(i) + j, fHSlider));
                    ...
                    fTabPage->AddFrame(fSliderFrame, new TGLayoutHints(kLHintsLeft | kLHintsTop | kLHintsExpandX, 2, 2, 2, 2));
                    {
                        ...
                        TGTextButton *fButtonTmp1 = new TGTextButton(fSliderFrame, "Pre Channel <", gMyGuiMainAction->GetIndexButton(i) + j + 1);
                        ...
                        butList.insert(pair<int, TGTextButton *>(gMyGuiMainAction->GetIndexButton(i) + j + 1, fButtonTmp1));

                        TGTextButton *fButtonTmp2 = new TGTextButton(fSliderFrame, "Next Channel >", gMyGuiMainAction->GetIndexButton(i) + j + 2);
                        ...
                        butList.insert(pair<int, TGTextButton *>(gMyGuiMainAction->GetIndexButton(i) + j + 2, fButtonTmp2));
                    }
                    j = j + 2;
                }
                else
                {
                    TGTextButton *fButtonTmp = new TGTextButton(fTabPage, gMyGuiMainAction->GetPageButtonName(i, j), gMyGuiMainAction->GetIndexButton(i) + j);
                    fButtonTmp->Associate(this);
                    fTabPage->AddFrame(fButtonTmp, LayoutX);
                    if (gMyGuiMainAction->GetPageButtonName(i, j).BeginsWith("Load"))
                        fButtonTmp->ChangeBackground(green);
                    if (gMyGuiMainAction->GetPageButtonName(i, j).BeginsWith("Set"))
                        fButtonTmp->ChangeBackground(yellow);
                    if (gMyGuiMainAction->GetPageButtonName(i, j).BeginsWith("Draw"))
                        fButtonTmp->ChangeBackground(yellow);
                    butList.insert(pair<int, TGTextButton *>(gMyGuiMainAction->GetIndexButton(i) + j, fButtonTmp));
                }
            }
        }
    }
...
Bool_t MyGuiMainFrame::ProcessMessage(Long_t msg, Long_t parm1, Long_t parm2)
{
    ...
    case kC_COMMAND:
        ...
        case kCM_MENU:
            ...
            if (parm1 == ReadRawData || parm1 == GenDSTRoot || parm1 == CombineDSTRoot || parm1 == LoadDSTRoot)
            {
                int retval;
